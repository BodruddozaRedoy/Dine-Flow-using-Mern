...
const AllFoodsPage = () => {
      ...
      const fetchSearch = async () => {
        ...
        try {
          const res = await axiosInstance.get(
            `/api/search-products?query=${search}`
          );
  ...
  const highlightText = (text, searchTerm) => {
    ...
    return text.replace(
      regex,
      (match) => `<span class="bg-primary">${match}</span>`
    );
  };
  ...
  const sorted = (e) => {
    ...
    setOrder(selected);
    axiosInstance
      .post(`/api/sorted-data?sortBy=price&order=${selected}`)
    ...
    <div>
      ...
      <div className="md:m-10 bg-white dark:bg-slate-400 rounded-xl p-5 md:p-10">
        <div className="flex flex-col md:flex-row items-center justify-between mb-5">
          <h1 className="font-serif font-bold text-2xl mb-4 md:mb-0 md:text-3xl">
            All Foods
          </h1>
          <div className="flex  items-center gap-3">
            ...
            <select
              ...
              id=""
              onChange={sorted}
              className="bg-primary border-none py-3 w-1/3 px-3 md:px-5 text-white rounded-lg"
            >
              <option
              ...
              <option className="bg-white text-primary" value="dsc">
                Sort by Descending
              </option>
              <option className="bg-white text-primary" value="asc">
                Sort by Ascending
              </option>
            </select>
          </div>
        ...
        {foods?.length == 0 || !allData && <CardSkeleton />}
        <div className=" grid lg:grid-cols-4 grid-cols-1 mt-5 gap-5">
          {(searchResults?.length > 0 ? searchResults : allData)?.map(
            (food) => (
              <AllFoodCard
                key={food._id}
                food={food}
                ...
                searchTerm={search}
              /> // Ensure each food has a unique key
            )
          )}
