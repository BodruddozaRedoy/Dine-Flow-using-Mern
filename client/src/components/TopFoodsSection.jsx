...
const TopFoodsSection = () => {
    ...
    useEffect(() => {
        const fetchSort = async () => {
            const res = await axiosInstance.post(`/api/sorted-data?sortBy=${sortBy}&order=${order}&limit=${6}`)
            setFoods(res.data);
        }
        fetchSort()
    }, [])
    return (
        <div className='md:mx-10 mx-3'>
            ...
            <Link to={"/all-foods"}><div className='w-40 mt-10 mx-auto'><MainBtn text={"See All"}/></div></Link>
        </div>
    );
};
